...
# init table
for FILE_PATH in `ls -1 /var/migrate/mwconfig/init/*.sql | sort -g`
do
...
# migrate data
for FILE_PATH in $(find /var/migrate/mwconfig/data -name '*.sql' | sort -g)
do
    ...
    su - db2inst1 -c "db2 -qtvf /tmp/migrate.sql" 2>&1 | while read -r line; do
        if [[ $line == INSERT* ]]; then
            ...
            $line == *SQL0530N* || 
            $line == *SQL0087N* || 
            $line == *"is not equal to any value of the"* ||
            $line == *"The command was processed as an SQL statement"* ||
