...
CREATE OR REPLACE PROCEDURE MERGE_CLIENT (
	...
	IN IDLE_WAIT BIGINT,
	IN TIMEOUT SMALLINT,
	IN CLOSE_QUEUE VARCHAR(256) DEFAULT NULL,
	IN CONTROL_QUEUE VARCHAR(256) DEFAULT NULL,
...
BEGIN
	...
	SET SQL_STMT = 'MERGE INTO MWCONFIG.CLIENT AS T ' ||
					...
					CASE WHEN IDLE_WAIT IS NOT NULL THEN CAST(IDLE_WAIT AS VARCHAR(20)) ELSE 'NULL' END || ', ' ||
					CASE WHEN TIMEOUT IS NOT NULL THEN CAST(TIMEOUT AS VARCHAR(20)) ELSE 'NULL' END || ', ' ||
					...
					CASE WHEN KEEP_CONN IS NOT NULL THEN CAST(KEEP_CONN AS VARCHAR(20)) ELSE 'NULL' END || ', ' ||
					CASE WHEN REUSE_DELAY IS NOT NULL THEN CAST(REUSE_DELAY AS VARCHAR(20)) ELSE 'NULL' END || ', ' ||
					...
					'VALUES (S.ADAPTOR_ID, S.IDLE_WAIT, S.TIMEOUT, S.CLOSE_QUEUE, S.CONTROL_QUEUE, S.KEEP_CONN, S.REUSE_DELAY, S.NODE)';
